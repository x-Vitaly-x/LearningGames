...
import { ROWS, ROW_NAMES } from '../katakana';
import { MAX_LIVES, LIFE_REGEN_EVERY } from '../../hiragana/constants';
import { Lives } from '../../hiragana/components/Lives';
...
function choiceClass(
  ...
  return base + 'bg-white border-slate-300 text-slate-700 hover:bg-teal-50 hover:border-teal-400 hover:text-teal-700';
}
...
export function QuizScreen({ state, dispatch }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full p-6 gap-6 max-w-md mx-auto w-full">
      ...
      {toastRowIndex !== null && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-teal-600 text-white px-6 py-4 rounded-2xl shadow-xl flex flex-col items-center gap-2 z-50"
        ...
        >
          ...
          <div className="text-sm text-teal-200">{ROW_NAMES[toastRowIndex]} row</div>
          <button
            onClick={() => dispatch({ type: 'DISMISS_UNLOCK' })}
            className="text-xs text-teal-300 hover:text-white mt-1 transition-colors"
          >
            Dismiss
          </button>
        </div>
