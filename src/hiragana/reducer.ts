import type { HiraganaState, Action, GameMode } from './types';
...
import { ALL_CHARS, ROWS } from './hiragana';

const ALL_ROW_INDICES = ROWS.map((_, i) => i);

export function freshSession(highScore: number, mode: GameMode = 'basic'): HiraganaState {
  return {
    screen: 'intro',
    mode,
    lives: MAX_LIVES,
...
function pickNext(state: HiraganaState) {
...
export function reducer(state: HiraganaState, action: Action): HiraganaState {
  switch (action.type) {
    case 'START_QUIZ': {
    ...
    case 'ANSWER': {
      ...
      const newSessionCorrect = {
        ...state.sessionCorrect,
        [kana]: (state.sessionCorrect[kana] ?? 0) + 1,
      };
      ...
      return {
        ...
        lives: newLives,
        retiredKana: newRetired,
        unlockedRowIndices: newUnlocked,
        toastRowIndex,
        lastResult: 'correct',
      };
    }
